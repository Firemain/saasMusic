...
class Audio extends AudioBase {
  ...
  details: { name: string; volume: number };

  constructor(props: AudioProps) {
    super(props);
    // Initialize default values for details
    this.details = {
      ...(typeof this.details === 'object' && this.details !== null ? this.details : {}),
      name: "Audio Track",
      volume: 50
    };
  }
  ...
  public drawTextIdentity(ctx: CanvasRenderingContext2D) {
    ...
    // Display the actual file name with fallback
    const name = this.details?.name || "Audio Track";
    const truncatedName = name.length > 20 ? name.substring(0, 17) + "..." : name;
    ctx.fillText(truncatedName, 36, 14);
  ...
  private drawVolumeControl(ctx: CanvasRenderingContext2D) {
    if (this.selected) {
      ctx.save();
      ctx.translate(-this.width / 2, this.height / 2 - 20);
      
      // Draw volume label with fallback
      ctx.font = "400 10px 'DM Sans'";
      ctx.fillStyle = "#f4f4f5";
      ctx.textAlign = "left";
      ctx.fillText(`Volume: ${Math.round(this.details?.volume || 50)}%`, 10, 0);
